...
TEST_F(InstructionTest, CMP_W) {
  ...
  auto& state = GetState();
  state.SetRegisters(
      {{CpuCore::ST, 0}, {CpuCore::R1, 0xFFFF}, {CpuCore::R2, 0x8000}});

  state.code.AddValue(Encode("CMP.W", CpuCore::R0, "$v")).AddValue(1);
  const uint16_t ip1 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.W", CpuCore::R1, "$v")).AddValue(0x7FFF);
  const uint16_t ip2 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.W", CpuCore::R2, "$v")).AddValue(1);
  const uint16_t ip3 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.W", CpuCore::R0, "$v")).AddValue(0);
  const uint16_t ip4 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.W", CpuCore::R0, "$v")).AddValue(0x8000);
  const uint16_t ip5 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("HALT"));

  ASSERT_TRUE(ExecuteUntilIp(ip1));  // CMP.W R0, 1
  EXPECT_EQ(state.r0, 0);
  EXPECT_EQ(state.st, CpuCore::S | CpuCore::C);
  ASSERT_TRUE(ExecuteUntilIp(ip2));  // CMP.W R1, 0x7FFF
  EXPECT_EQ(state.st, CpuCore::S);
  ASSERT_TRUE(ExecuteUntilIp(ip3));  // CMP.W R2, 1
  EXPECT_EQ(state.st, CpuCore::O);
  ASSERT_TRUE(ExecuteUntilIp(ip4));  // CMP.W R0, 0
  EXPECT_EQ(state.st, CpuCore::Z);
  ASSERT_TRUE(ExecuteUntilIp(ip5));  // CMP.W R0, 0x8000
  EXPECT_EQ(state.st, CpuCore::S | CpuCore::C | CpuCore::O);
}

TEST_F(InstructionTest, CMP_D) {
  ASSERT_TRUE(InitAndReset());
  auto& state = GetState();
  state.SetRegisters({{CpuCore::ST, 0},
                      {CpuCore::R2, 0xFFFF},
  ...
  state.code.AddValue(Encode("CMP.D", 0, "$V")).AddValue32(1);
  const uint16_t ip1 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.D", 1, "$V")).AddValue32(0x7FFFFFFF);
  const uint16_t ip2 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.D", 2, "$V")).AddValue32(1);
  const uint16_t ip3 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.D", 0, "$V")).AddValue32(0);
  const uint16_t ip4 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("CMP.D", 0, "$V")).AddValue32(0x80000000);
  const uint16_t ip5 = state.code.AddNopGetAddress();
  state.code.AddValue(Encode("HALT"));

  ASSERT_TRUE(ExecuteUntilIp(ip1));  // CMP.D D0, 1
  EXPECT_EQ(state.d0(), 0);
  EXPECT_EQ(state.st, CpuCore::S | CpuCore::C);
  ASSERT_TRUE(ExecuteUntilIp(ip2));  // CMP.D D1, 0x7FFFFFFF
  EXPECT_EQ(state.st, CpuCore::S);
  ASSERT_TRUE(ExecuteUntilIp(ip3));  // CMP.D D2, 1
  EXPECT_EQ(state.st, CpuCore::O);
  ASSERT_TRUE(ExecuteUntilIp(ip4));  // CMP.D D0, 0
  EXPECT_EQ(state.st, CpuCore::Z);
