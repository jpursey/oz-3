...
TEST_F(InstructionTest, SHL_W_OneByValue) {
  ...
  for (int i = 1; i < 15; ++i) {
    ...
    EXPECT_EQ(state.st, 0) << "SHL.W R0, " << i;
  }
...
TEST_F(InstructionTest, SHL_W_OneByRegister) {
  ASSERT_TRUE(InitAndReset());
  auto& state = GetState();
  state.SetRegisters({{CpuCore::ST, 0}});
  ...
  EXPECT_EQ(state.r0, 1) << "SHL.W R0, R1=0";
  EXPECT_EQ(state.st, 0) << "SHL.W R0, R1=0";
  for (int i = 1; i < 15; ++i) {
    ASSERT_TRUE(ExecuteUntilIp(ips[i]));
    EXPECT_EQ(state.r0, 1 << i) << "SHL.W R0, R1=" << i;
    EXPECT_EQ(state.st, 0) << "SHL.W R0, R1=" << i;
  }
  ASSERT_TRUE(ExecuteUntilIp(ips[15]));
...
TEST_F(InstructionTest, SHL_D_OneByValue) {
  ASSERT_TRUE(InitAndReset());
  auto& state = GetState();
  state.SetRegisters({{CpuCore::ST, 0}});
  ...
  for (int i = 1; i < 31; ++i) {
    ...
    EXPECT_EQ(state.st, 0) << "SHL.D D0, " << i;
  }
...
TEST_F(InstructionTest, SHL_D_OneByRegister) {
  ASSERT_TRUE(InitAndReset());
  auto& state = GetState();
  state.SetRegisters({{CpuCore::ST, 0}});
  ...
  ASSERT_TRUE(ExecuteUntilIp(ips[0]));
  EXPECT_EQ(state.d0(), 1) << "SHL.D D0, R2=0";
  EXPECT_EQ(state.st, 0) << "SHL.D D0, R2=0";
  for (int i = 1; i < 31; ++i) {
    ASSERT_TRUE(ExecuteUntilIp(ips[i]));
    EXPECT_EQ(state.d0(), 1 << i) << "SHL.D D0, R2=" << i;
    EXPECT_EQ(state.st, 0) << "SHL.D D0, R2=" << i;
  }
  ASSERT_TRUE(ExecuteUntilIp(ips[31]));
